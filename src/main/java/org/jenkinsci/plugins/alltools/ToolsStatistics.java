 ...
 */
@ExportedBean
public class ToolsStatistics implements Serializable {
	/** Serial version UID. */
	...
	/** Count of falseAlarm issues. */
	private final int falseAlarmCount;
	 ...
	 */
	public ToolsStatistics() {
		this(0, 0, 0, 0, 0, 0, 0, 0, Collections.<String> emptySet());
	}
	 ...
	 *            Cppcheck versions used for generating of the report
	 */
	public ToolsStatistics(int errorCount, int warningCount, int styleCount,
			int performanceCount, int informationCount, int noCategoryCount,
			int portabilityCount, int falseAlarmCount, Set<String> versions) {
	...
	public int getNumberTotal() {
		return errorCount + warningCount + styleCount + performanceCount
				+ informationCount + noCategoryCount + portabilityCount + falseAlarmCount;
	}
